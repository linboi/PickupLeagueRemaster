...
class Match:
    ...
    def get_details_string(self):
        discordIDs = ", ".join(
            str(p.get_dID())
            for p in self.blueTeam.get_player_list() + self.redTeam.get_player_list()
        )
        res = self.cursor.execute(
            f"SELECT [name] FROM Account JOIN Player ON Account.playerID = Player.playerID WHERE Player.discordID in ({discordIDs})"
        ).fetchall()
        invite_strings = []
        idx = 0
        while idx < len(res):
    ...
    async def replacePlayer(self, discordID, otherID, channel, client):
        listOfPlayers = [
            self.redTeam.get_player_list() + self.blueTeam.get_player_list()
        ]
    ...
    async def openBetting(self, message):
        ...
        await message.edit(
            content=(
                matchMessage
                + "\nBetting closes:<t:"
                + str(int(time.mktime(closingTime.timetuple())))
                + ":R>"
            )
        )
        ...
        while not bettingClosed:
            try:
                team, user = await self.client.wait_for(
                    "reaction_add",
                    check=check,
                    timeout=(closingTime - datetime.datetime.now()).total_seconds(),
                )
                teamChosen = ""
                if team.emoji == "🔵":
                    teamChosen = "BLUE"
    ...
    async def respondToBet(self, user, team, closingTime):
        ...
        await user.send(
            f"How much do you want to bet on team {team} in match {self.matchID} (current balance: {balance:.0f}):"
        )
        ...
        if team == "BLUE":
            ...
            if user.id in self.redBets:
                ...
                self.redBets[user.id] = amount
        await user.send(
            f"Successfully placed a bet of {amount:.0f} on team {team} in match {self.matchID}!"
        )

    async def linkResult(self, message):
        if self.match_message is not None:
            msg = self.match_message.content
            await self.match_message.edit(
