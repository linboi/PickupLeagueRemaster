...
class Match:
    ...
    def orderBasedOnMMR(self, selectedPlayerList):
        ...
        ordered_rank_list = sorted(
            selectedPlayerList, key=lambda p: p.get_rating(), reverse=True)
    ...
    def fitRoles(self, players):
        ...
        for player in fill_players:
            available_roles = [role for role in assigned_roles.keys() if len(
                assigned_roles[role]) < 2]
            if available_roles:
    ...
    def insert(self):
        ...
        self.cursor.execute(
            f"INSERT INTO Match (matchID , matchTime) VALUES ({matchid}, 'TODAY')")
        self.con.commit()
    ...
    def update(self, ratingchange, winner):
        ...
        csvnames += f"{ratingchange*-1*winner}, " * \
            4 + f"{ratingchange*-1*winner}"
