...
def getHighestMMR(rankAndOffsetList):
    return max(
        (getMMRFromRank(tier, div, lp) + offset)
        for tier, div, lp, offset in rankAndOffsetList
    )
...
def getMMRFromRank(rankTier, rankDiv, rankLP):
    ...
    # Div Mappings
    mappingDivs = {"1": 300, "2": 200, "3": 100, "4": 0}
...
async def scrapeRanksFromLOG(gamename, tagline):
    ...
    for box in div_tags:
        ...
        for match in re.compile(
            r"(.+?)This player reached (.+?) during (.+?)\. At the end of the season, this player was (.+?)\."
        ).finditer(description.get_text()):
