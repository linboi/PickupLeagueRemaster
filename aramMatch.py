...
class ARAM_Match:
    ...
    def __init__(
        ...
        with open("./champList.txt", "r") as f:
            ...
            self.redChamps = champList[
                ARAM_Match.champsPerTeam : 2 * ARAM_Match.champsPerTeam
            ]
    ...
    def get_details_string(self):
        ...
        for i, c in enumerate(self.blueChamps):
            string += (
                f"{self.blueChamps[i].upper():^15}\t{self.redChamps[i].upper():^15}\n"
            )
        string += "```"
    ...
    def listOfUsers(self):
        listOfPlayers = [self.redTeam.players + self.blueTeam.players]
        listOfUsers = []
