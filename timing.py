...
async def sleep_until(schedule):
	...
	for gameday in gamedays:
		...
		if (not soonest_gameday or gameday_datetime < soonest_gameday) and (weekday > now.weekday() or (weekday == now.weekday() and gameday_datetime > now)):
			soonest_gameday = gameday_datetime.replace(day=now.day + (weekday - now.weekday()) % 7)
	...
	if soonest_gameday:
		...
		weekday, time_str = gamedays[0] # Assume the first game day is the next game day
		time_obj = datetime.datetime.strptime(time_str, "%H:%M").time()
		next_gameday_datetime = datetime.datetime.combine(now.date(), time_obj) + datetime.timedelta(days=(7 - now.weekday() + weekday) % 7)
		...
		if seconds_until_next_gameday < 0:
			seconds_until_next_gameday = (next_gameday_datetime.replace(day=next_gameday_datetime.day+7) - now).total_seconds()
