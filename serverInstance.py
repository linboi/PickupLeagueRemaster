...
")
    ...
    async def punishPlayer(self, message_obj, discordID):
        ...
        if admin_check:
            ...
            self.cursor.execute(
                f"UPDATE Player SET leaderboardPoints = {new_mmr}, signupCount = {add_signupCount} WHERE discordID = {discordID}")
            self.con.commit()
        ...
        for x in range(team_count**5):
            ...
            if maxDiffBetweenTeams < bestMaxMMRdiff:
                ...
                while idx+1 < len(teamList):
                    ...
                    bestMatches.append(Match(self.cursor, self.con, matchID=len(bestMatches) + 1 + datetime.datetime.now().hour + datetime.datetime.now(
                    ).second, blueTeam=blueTeam, redTeam=redTeam, startTime=str(datetime.datetime.now().date()) + ", " + str(datetime.datetime.now().hour) + ":00"))
                    idx += 2
        print(
